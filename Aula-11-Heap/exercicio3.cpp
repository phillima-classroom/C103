...
void heapSort(Aluno *V, int N){
    ...
    //Os demais elementos são filhos. 
    //Ao final desse processo o pai da árvore é o maior elemento
    for(int i = (N-1)/2; i >= 0; i--){
        criaHeap(V, i, N-1);
    }
    //Começa pegar o pai (que é o primeiro elemento) e 
    //colocar na última posição. Decrementamos o "i",
    //pois a última posição já estará ordenada.
    for(int i = N-1; i>= 1; i--){
        aux = V[0];
        V[0] = V[i];
        V[i] = aux;
        //Depois de trocarmos os valores 
        //(colocar o pai no fim do vetor), recriamos a heap, para buscar o maior elemento.
        criaHeap(V, 0, i-1);
    }
}
