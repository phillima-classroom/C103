...
//Essa função é responsável por calcular o pivô, e garantir que os elementos a esquerda 
//são menores que o pivô, e a direita maior.
//Assim, duas partições são criadas. E de forma recursiva o algoritmo 
//será executado para ambas as partições.
int particiona(int *V, int inicio, int fim){
   int esq,dir,pivo,aux;
   esq = inicio; //Por padrão a esq começa no início da partição
   dir = fim; //E a direta no final
   
   //Mudanca de inicio com fim
   aux = V[fim];
   V[fim] = V[inicio];
   V[inicio] = aux;
   
   //O pivô é calculado como sendo o primeiro valor da partição (lembrando que ja enganamos o algoritmo). 
   //Após a execução, o algoritmo irá coloca-lo na posição mais adequada,
   //garantindo que todos a esquerda são menor. 
   //E a direita maior.
   pivo = V[inicio]; 

    //A valor da esquerda é menor que a direita? 
    //Se sim, então continuamos a percorrer o vetor
    while (esq < dir){
        
        //O valor vetor na posição "esq" é menor que o pivô? 
        //Se sim, continuamos andando. (incrementa a esq)
        while (V[esq] <= pivo && esq < fim){ 
            esq++;
        }
        //O valor do vetor na posição da direita é maior que o pivo? 
        //Se sim, continuamos descendo a posição dir (dir --)
        while(V[dir] > pivo){ 
            dir--;
        }
        //Se a esq for menor que a direita,significa que o valor na posição dir é menor
        // que o da posição esq, por isso trocamos.
        if(esq < dir){ 
            aux = V[esq];
            V[esq] = V[dir];
            V[dir] = aux;
