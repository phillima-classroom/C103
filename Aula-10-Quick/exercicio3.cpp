...
int particiona(Aluno *V, int inicio, int fim){
   ...
   //O pivô é calculado como sendo o primeiro valor da partição. 
   ...
   pivo = V[inicio]; 
    ...
    while (esq < dir){
        ...
        while (V[esq].nome <= pivo.nome && esq < fim){ 
            esq++;
        }
        //O valor do vetor na posição da direita é maior que o pivo? 
        //Se sim, continuamos descendo a posição dir (dir --)
        while(V[dir].nome > pivo.nome){ 
            dir--;
        }
        //Se a esq for menor que a direita,significa que o valor na 
        //posição dir é menor que o da posição esq, por isso trocamos.
        if(esq < dir){ 
            aux = V[esq];
            V[esq] = V[dir];
            V[dir] = aux;
